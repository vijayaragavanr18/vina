...
class DesktopModule:
    ...
    async def run(self, target: TargetInput) -> DesktopResult:
        ...
        gdm_configs = ["/etc/gdm3/custom.conf", "/etc/gdm/custom.conf", "/etc/gdm3/daemon.conf"]
        for cfg in gdm_configs:
            ...
            if cr_gdm.succeeded and cr_gdm.stdout.strip():
                ...
                if "AutomaticLoginEnable=true" in content.replace(
                    " ", ""
                ) or "TimedLoginEnable=true" in content.replace(" ", ""):
                    findings.append(
                        make_finding(
        ...
        lightdm_configs = ["/etc/lightdm/lightdm.conf"]
        find_cmd = self.config.tool_bin("find", "find")
        cr_find_ld = await self.context.runner.run(
            find_cmd, ["/etc/lightdm/lightdm.conf.d", "-type", "f"], timeout_seconds=5
        )
