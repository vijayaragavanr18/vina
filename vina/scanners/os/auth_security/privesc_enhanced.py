...
class PrivescEnhancedModule:
    ...
    @staticmethod
    async def _check_sgid(results: dict[str, CommandResult], findings: list[Finding], target: str) -> None:
        cr = results.get("sgid")
    ...
    @staticmethod
    async def _check_writable_scripts(results: dict[str, CommandResult], findings: list[Finding], target: str) -> None:
        cr = results.get("writable_scripts")
        if cr is None or not cr.succeeded or not cr.stdout.strip():
            return
        paths = [line.strip() for line in cr.stdout.splitlines() if line.strip()]
        interesting = [
            p for p in paths if any(p.startswith(prefix) for prefix in ("/etc/", "/usr/", "/opt/", "/root/", "/var/"))
        ]
        if interesting:
            evidence = "\n".join(interesting[:15])
    ...
    @staticmethod
    async def _check_systemd_timers(results: dict[str, CommandResult], findings: list[Finding], target: str) -> None:
    ...
    async def _check_path_hijacking(
        self, _results: dict[str, CommandResult], findings: list[Finding], target: str, _warnings: list[str]
    ) -> None:
        cr = await self.context.runner.run(
            self.config.tool_bin("echo", "echo"), ["$PATH"], timeout_seconds=self.context.timeout_seconds
        )
        if cr.succeeded and cr.stdout.strip():
            ...
            for pdir in path_dirs:
                stat_cr = await self.context.runner.run(
                    self.config.tool_bin("stat", "stat"), ["--format=%a", pdir], timeout_seconds=5
                )
        ...
        ldr = await self.context.runner.run(self.config.tool_bin("echo", "echo"), ["$LD_PRELOAD"], timeout_seconds=5)
        ...
        ldp = await self.context.runner.run(
            self.config.tool_bin("echo", "echo"), ["$LD_LIBRARY_PATH"], timeout_seconds=5
        )
    ...
    @staticmethod
    async def _check_ld_preload(results: dict[str, CommandResult], findings: list[Finding], target: str) -> None:
        pass

    @staticmethod
    def _check_gtfobins_suid(results: dict[str, CommandResult], findings: list[Finding], target: str) -> None:
    ...
    async def _check_process_capabilities(self, findings: list[Finding], target: str) -> None:
        cap_cr = await self.context.runner.run(
            self.config.tool_bin("capsh", "capsh"), ["--print"], timeout_seconds=self.context.timeout_seconds
        )
        if cap_cr.succeeded and cap_cr.stdout.strip():
            ...
            if (
                ambient_set
                and ambient_set != "="
                and not ambient_set.lower().startswith("empty")
                and ambient_set.strip() != ""
            ):
                findings.append(
                    make_finding(
