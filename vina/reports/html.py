...
def _collapse_section(_section_id: str, title: str, content: str, open_default: bool = True) -> str:
...
def _findings_table(flist: Sequence[Finding | EnrichedFinding], show_evidence: bool = True) -> str:
    if not flist:
...
def _attack_path_body(path: AttackPath) -> str:
    lines = ""
    ...
    if path.evidence:
        ev_lines = "".join(f"<li>{_esc(_line)}</li>" for _line in path.evidence.split("\n"))
...
def _attack_path_meta(path: AttackPath) -> str:
    meta = ""
...
def _attack_path_card(path: AttackPath, idx: int) -> str:
    ...
    lines = _attack_path_body(path)
    meta = _attack_path_meta(path)
...
def _detailed_finding_body(f: Finding | EnrichedFinding) -> str:
    sections = ""
    if isinstance(f, EnrichedFinding):
        if f.explanation:
    ...
    rem = _esc(f.remediation) if isinstance(f, EnrichedFinding) and f.remediation else ""
    if rem:
        sections += f"<p><strong>Remediation:</strong> {rem}</p>"
    if f.recommendation and not rem:
        sections += f"<p><strong>Recommendation:</strong> {_esc(f.recommendation)}</p>"
    return sections


def _detailed_finding_meta(f: Finding | EnrichedFinding) -> str:
    meta = ""
    if isinstance(f, EnrichedFinding):
        if f.cis_control:
...
def _detailed_finding_card(f: Finding | EnrichedFinding, idx: int) -> str:
    color = _severity_color(f.severity)
    sections = _detailed_finding_body(f)
    meta = _detailed_finding_meta(f)
    meta_html = f"<ul>{meta}</ul>" if meta else ""
...
def _exploitability_assessment_card(a: ExploitabilityAssessment, idx: int) -> str:
    ...
    if a.missing_prerequisites:
        meta += (
            f"<li><strong>Missing Prerequisites:</strong> {', '.join(_esc(p) for p in a.missing_prerequisites)}</li>"
        )
    ...
    return f"""<div class='exploit-card' style='border-left:4px solid {score_color}' id='exploit-{idx}'>
  <h4>{_esc(a.title)}</h4>
  {lines}
  {meta_html}
</div>"""
...
def _exploitability_section(
    ...
    return _collapse_section(
        "exploitability", f"Exploitability Analysis ({len(assessments)})", content, open_default=bool(assessments)
    )


def _severity_stat_cards(stats: AggregatorStats) -> str:
...
def _attack_path_stat_cards(stats: AggregatorStats) -> str:
...
def _summary_stat_cards(stats: AggregatorStats, findings: Sequence[Finding | EnrichedFinding]) -> str:
...
def _stage_table_html(stage_results: list[StageResult]) -> str:
    rows = ""
...
def _findings_by_severity_html(sev_groups: dict[str, list[Finding]]) -> str:
    severity_order = ["critical", "high", "medium", "low", "info"]
    sections = ""
    for sev in severity_order:
        ...
        sections += _collapse_section(f"sev-{sev}", f"{sev.title()} ({len(flist)})", content)
    if not sections:
        sections = "<p class='empty'>No findings recorded.</p>"
    return sections


def _findings_by_category_html(cat_groups: dict[str, list[Finding]]) -> str:
    sections = ""
    for cat in sorted(cat_groups.keys()):
        flist = cat_groups[cat]
        content = _findings_table(flist, show_evidence=False)
        sections += _collapse_section(
            f"cat-{cat}", f"{cat.replace('_', ' ').title()} ({len(flist)})", content, open_default=False
        )
    if not sections:
        sections = "<p class='empty'>No categories.</p>"
    return sections


def _detailed_findings_html(findings: Sequence[Finding | EnrichedFinding]) -> str:
    cards = ""
    for idx, f in enumerate(findings):
        if _is_enriched(f):
            ...
            if card:
                cards += card
    if not cards:
        cards = "<p class='empty'>No enriched findings to display.</p>"
    return cards


def _recommendations_html(findings: Sequence[Finding | EnrichedFinding]) -> str:
    items = ""
    for f in findings:
        rem = f.remediation if isinstance(f, EnrichedFinding) and f.remediation else f.recommendation
        if not rem:
            continue
        color = _severity_color(f.severity)
        refs = "".join(f"<li>{_esc(r)}</li>" for r in f.references)
        if isinstance(f, EnrichedFinding) and f.enriched_references:
    ...
    if not items:
        items = "<p class='empty'>No specific recommendations were generated.</p>"
    return items


def _appendix_html(stage_results: list[StageResult]) -> str:
    items = ""
    for sr in stage_results:
        status = sr.status.value if hasattr(sr.status, "value") else str(sr.status)
        warnings = ""
...
def _vulnerability_matches_rows(vmatches: list[VulnerabilityMatch]) -> str:
    return "".join(
        f"<tr>"
...
def render_html(
    ...
    **_kwargs: Any,
    ...
    sev_cards = _severity_stat_cards(stats)
    attack_cards = _attack_path_stat_cards(stats)
    vuln_cards = _vulnerability_stat_cards(vmatches, vuln_stats)
    summary_cards = _summary_stat_cards(stats, findings)
    ...
    stage_table = _stage_table_html(stage_results)
    sev_sections = _findings_by_severity_html(sev_groups)
    cat_sections = _findings_by_category_html(cat_groups)
    detail_cards = _detailed_findings_html(findings)
    rec_items = _recommendations_html(findings)
    appendix_items = _appendix_html(stage_results)
...
<body>
  <main>
...

    {
        _collapse_section(
            "vulnerabilities",
            f"Known Vulnerabilities ({len(vmatches)})",
            f'''
    ...
    <table>
      ...
      <tbody>
        {_vulnerability_matches_rows(vmatches)}
      </tbody>
    </table>
    ''',
    ...
    {
        _collapse_section(
            "attack-paths", f"Attack Paths ({stats.attack_paths_total})", attack_path_content, open_default=bool(paths)
        )
    }
